...
import { VoucherDialog } from './create-voucher'

interface VouchersProps {
  programs: any[]
}

export default function Vouchers({ programs }: VouchersProps) {
    ...
    <div className='flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-y-0'>
      <div className='my-4 flex items-end justify-between sm:my-0 sm:items-center'>
        <div className='flex flex-col gap-4 sm:my-4 sm:flex-row'>
          <Input
            placeholder='Filter vouchers/coupons...'
          ...
          .map((program, idx) => (
            ...
            >
              <div className='mb-8 flex items-center justify-between'>
                <h1 className='mb-1 font-semibold'>{program.title}</h1>
              ...
              <div className='flex items-start justify-between'>
                <div>
                  <h2 className='mb-1'>{program.message}</h2>
                  <p className='line-clamp-2 text-gray-500'>
                    Nº de Stamps: {program.stamps}
                  </p>
                </div>
                <span>
                  <Pencil2Icon
      ...
      <VoucherDialog open={showDialog} onOpenChange={setShowDialog} />
    </div>
  )
}
